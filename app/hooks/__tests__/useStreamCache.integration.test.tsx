...
describe("useStreamCache BG fetch", () => {
  ...
  it("fetches and aligns BG per run, producing glucose data", async () => {
    ...
    const persisted = JSON.parse(store.get("bgcache_v8") ?? "[]");
  ...
  it("preserves unrelated persisted cache rows when syncing a subset of runs", async () => {
    ...
    store.set("bgcache_v8", JSON.stringify([makeCachedActivity("act-2")]));
    ...
    const persisted = JSON.parse(store.get("bgcache_v8") ?? "[]") as CachedActivity[];
