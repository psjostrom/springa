...
describe("SettingsModal totalWeeks validation", () => {
  ...
  it("base phase toggle is disabled when totalWeeks is below 11", () => {
    ...
    const basePhaseSwitch = switches.find(s => s.getAttribute("aria-label")?.includes("base") || s.closest("label")?.textContent?.includes("Base"));
    // Fall back to the first switch that's disabled if we can't find by label
    const target = basePhaseSwitch ?? switches.find(s => (s as HTMLButtonElement).disabled);
  ...
  it("base phase toggle is enabled when totalWeeks is 11 or more", () => {
    renderModal({ totalWeeks: 12 });
    const switches = screen.getAllByRole("switch");
    // The base phase switch should be enabled; find the one near "Base phase" text
    const basePhaseSwitch = switches.find(s => s.getAttribute("aria-label")?.includes("base") || s.closest("label")?.textContent?.includes("Base"));
    const target = basePhaseSwitch ?? switches[0];
