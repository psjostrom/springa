    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
          isUser
            ? "bg-brand/20 border border-brand/30 text-text whitespace-pre-wrap"
        ...
        {isUser ? content : (
          <ReactMarkdown
            ...
            components={{
              ...
              h2: ({ children }) => <h2 className="text-base font-bold text-text mb-1">{children}</h2>,
              h3: ({ children }) => <h3 className="text-sm font-bold text-text mb-1">{children}</h3>,
              code: ({ children }) => <code className="bg-surface-alt px-1 py-0.5 rounded text-brand text-xs">{children}</code>,
              pre: ({ children }) => <pre className="bg-surface-alt rounded p-2 mb-2 last:mb-0 overflow-x-auto text-xs">{children}</pre>,
              ...
              th: ({ children }) => <th className="text-left px-2 py-1 text-text font-semibold">{children}</th>,
