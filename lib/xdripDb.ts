...
// --- Strimma readings (identical schema, separate table) ---

export async function getStrimmaReadings(
  email: string,
  months?: string[],
): Promise<XdripReading[]> {
  if (!months) {
  ...
  const result = await db().execute({
    sql: "SELECT ts, mmol, sgv, direction FROM strimma_readings WHERE email = ? AND ts >= ? AND ts < ? ORDER BY ts",
    args: [email, minTs, maxTs],
  });
...
export async function saveStrimmaReadings(
  email: string,
  readings: XdripReading[],
): Promise<void> {
  if (readings.length === 0) return;
  ...
  for (let i = 0; i < readings.length; i += BATCH_SIZE) {
    ...
    await db().batch(
      chunk.map((r) => ({
        sql: `INSERT OR REPLACE INTO strimma_readings (email, ts, mmol, sgv, direction)
              VALUES (?, ?, ?, ?, ?)`,
        args: [email, r.ts, r.mmol, r.sgv, r.direction],
      })),
      "write",
...
export async function getRecentStrimmaReadings(
  email: string,
  withinMs: number = 30 * 60 * 1000,
): Promise<XdripReading[]> {
  const cutoff = Date.now() - withinMs;
  const result = await db().execute({
    sql: "SELECT ts, mmol, sgv, direction FROM strimma_readings WHERE email = ? AND ts >= ? ORDER BY ts",
    args: [email, cutoff],
  });
  return result.rows.map((row) => ({
    ts: row.ts as number,
