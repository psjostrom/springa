...
export function classifyEntryLabel(slope: number, stability: number): string {
  ...
  if (Math.abs(slope) <= 0.03 && stability < 0.5) return "stable";
...
export function formatRunLine(
  ...
  if (extras?.bgStartAndRate) {
    ...
    if (bg.entrySlope != null) {
      ...
      bgText += ` (entry ${slopeSign}${bg.entrySlope.toFixed(2)}/min)`;
    }
    bgText += ` | BG rate ${sign}${bg.avgRate.toFixed(3)}/min`;
    parts.push(bgText);
  }
  ...
  if (extras?.bgSummary) {
    ...
    if (bg.dropRate != null) bgParts.push(`drop ${bg.dropRate >= 0 ? "+" : ""}${bg.dropRate.toFixed(3)}/min`);
  ...
  if (extras?.runBGContext?.pre) {
    ...
    parts.push(`start BG ${pre.startBG.toFixed(1)} | entry: ${slopeSign}${pre.entrySlope30m.toFixed(2)}/min (${classifyEntryLabel(pre.entrySlope30m, pre.entryStability)})`);
  }
