...
describe("getPaceTable", () => {
  describe("Half Marathon (21.0975km)", () => {
    it("2h20 HM: easy 7.0-7.8, steady 6.5-6.7, tempo 6.0-6.2", () => {
      ...
      expect(result.z2.min).toBeGreaterThan(7.0);
      expect(result.z2.min).toBeLessThan(7.1);
      expect(result.z2.max).toBeGreaterThan(7.7);
      expect(result.z2.max).toBeLessThan(7.9);
      expect(result.z3.min).toBeGreaterThan(6.4);
      expect(result.z3.min).toBeLessThan(6.6);
      expect(result.z3.max).toBeGreaterThan(6.6);
      expect(result.z3.max).toBeLessThan(6.8);
      expect(result.z4.min).toBeGreaterThan(5.9);
      expect(result.z4.min).toBeLessThan(6.1);
      expect(result.z4.max).toBeGreaterThan(6.1);
      expect(result.z4.max).toBeLessThan(6.3);
    });

    it("2h00 HM: easy >6.0 <7.1, steady >5.5 <5.8", () => {
      const result = getPaceTable(21.0975, 7200);
      expect(result.z2.min).toBeGreaterThan(6.0);
      expect(result.z2.min).toBeLessThan(7.1);
      expect(result.z3.min).toBeGreaterThan(5.5);
      expect(result.z3.max).toBeLessThan(5.8);
    });

    it("3h00 HM: easy >8.9 <10.3", () => {
      const result = getPaceTable(21.0975, 10800);
      expect(result.z2.min).toBeGreaterThan(8.9);
      expect(result.z2.max).toBeLessThan(10.3);
    });
  });

  it("paces follow easy > steady > tempo > hard (for HM)", () => {
    const result = getPaceTable(21.0975, 8400);
    expect(result.z2.min).toBeGreaterThan(result.z3.max);
    expect(result.z3.min).toBeGreaterThan(result.z4.max);
    expect(result.z4.min).toBeGreaterThan(result.z5);
  });

  it("Ben Parkes 2h20 validation: easy.min ≈7.05, easy.max ≈7.77", () => {
    const result = getPaceTable(21.0975, 8400);
    expect(result.z2.min).toBeCloseTo(7.05, 0);
    expect(result.z2.max).toBeCloseTo(7.77, 0);
  });

  it("hard is a number (not a range)", () => {
    const result = getPaceTable(21.0975, 8400);
    expect(typeof result.z5).toBe("number");
    expect(result.z5).toBeGreaterThan(0);
  });
  ...
  it("55min 10K: steady uses actual 10K pace (~5.5), easy based on HM-equivalent", () => {
    ...
    expect(result.z3.min).toBeGreaterThan(5.3);
    expect(result.z3.min).toBeLessThan(5.5);
    expect(result.z3.max).toBeGreaterThan(5.5);
    expect(result.z3.max).toBeLessThan(5.7);

    // Easy should be based on HM-equivalent pace (slower than 10K pace)
    expect(result.z2.min).toBeGreaterThan(result.racePacePerKm);
  });
  ...
  it("4h Marathon: derives paces from HM-equivalent", () => {
    ...
    expect(result.z2.min).toBeGreaterThan(result.hmEquivalentPacePerKm);
  });
...
describe("getPaceTable steady zone", () => {
  it("steady uses ability pace", () => {
    ...
    expect(result.z3.max).toBeCloseTo(abilityPace * 1.01, 1);
  });
