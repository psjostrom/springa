...
describe("assessReadiness — trend slope", () => {
  it("wait when slope < -0.05 (dropping fast)", () => {
    const g = assessReadiness(makeInput({ trendSlope: -0.08 }));
    expect(g.level).toBe("wait");
    expect(g.reasons).toContain("BG dropping fast");
  });

  it("caution when slope -0.05 to -0.03 (BG above 8)", () => {
    const g = assessReadiness(makeInput({ currentBG: 9.0, trendSlope: -0.04 }));
    expect(g.level).toBe("caution");
    expect(g.reasons).toContain("BG trending down");
  });

  it("ready when slope stable (-0.03 to +0.03)", () => {
    const g = assessReadiness(makeInput({ trendSlope: 0.0 }));
    expect(g.level).toBe("ready");
  });

  it("ready when slope rising (> +0.03)", () => {
    const g = assessReadiness(makeInput({ trendSlope: 0.08 }));
    expect(g.level).toBe("ready");
  });
  ...
  it("exactly -0.05 is caution when BG above 8", () => {
    const g = assessReadiness(makeInput({ currentBG: 9.0, trendSlope: -0.05 }));
    expect(g.level).toBe("caution"); // -0.05 is >= -0.05, so it's in the -0.05 to -0.03 range
  });

  it("exactly -0.03 is caution", () => {
    const g = assessReadiness(makeInput({ trendSlope: -0.03 }));
    expect(g.level).toBe("caution");
    expect(g.reasons).toContain("BG trending down");
  });
});
...
describe("assessReadiness — 30-min forecast", () => {
  it("projects BG using current trend slope", () => {
    // slope -0.08/min → drop 2.4 in 30m → 7.5 - 2.4 = 5.1
    const g = assessReadiness(makeInput({ currentBG: 7.5, trendSlope: -0.08 }));
    expect(g.predictedDrop).toBeCloseTo(-2.4);
    expect(g.estimatedBGAt30m).toBeCloseTo(5.1);
  });

  it("shows rising forecast when trending up", () => {
    const g = assessReadiness(makeInput({ currentBG: 7.0, trendSlope: 0.05 }));
    expect(g.predictedDrop).toBeCloseTo(1.5);
    expect(g.estimatedBGAt30m).toBeCloseTo(8.5);
  });

  it("caution when trend predicts hypo", () => {
    // slope -0.13/min → drop 3.9 in 30m → 5.4 - 3.9 = 1.5
    const g = assessReadiness(makeInput({ currentBG: 5.4, trendSlope: -0.13 }));
...
describe("assessReadiness — combined", () => {
  it("low BG + dropping → wait", () => {
    const g = assessReadiness(makeInput({ currentBG: 4.2, trendSlope: -0.06 }));
    expect(g.level).toBe("wait");
  ...
  it("good BG + rising → ready", () => {
    const g = assessReadiness(makeInput({ currentBG: 7.5, trendSlope: 0.1 }));
    expect(g.level).toBe("ready");
  });
...
describe("assessReadiness — compound low+falling rule", () => {
  it("wait when BG < 8 and slope < -0.03", () => {
    const g = assessReadiness(makeInput({ currentBG: 7.5, trendSlope: -0.04 }));
    expect(g.level).toBe("wait");
    expect(g.reasons).toContain("BG below 8 and falling — high hypo risk");
  });

  it("caution (not wait) when BG >= 8 and slope < -0.03", () => {
    const g = assessReadiness(makeInput({ currentBG: 8.0, trendSlope: -0.04 }));
    expect(g.level).toBe("caution");
  ...
  it("does not trigger when slope is only mildly negative", () => {
    const g = assessReadiness(makeInput({ currentBG: 7.0, trendSlope: -0.02 }));
    expect(g.reasons).not.toContain("BG below 8 and falling — high hypo risk");
  });
});
...
describe("assessReadiness — compound low+falling with IOB", () => {
  it("wait when BG < 8, falling, and IOB >= 0.5 with consolidated carbs", () => {
    const g = assessReadiness(makeInput({ currentBG: 7.2, trendSlope: -0.04, iob: 1.0 }));
    expect(g.level).toBe("wait");
    expect(g.reasons).toContain("BG below 8 and falling — high hypo risk");
    expect(g.reasons.some((r) => r.includes("IOB"))).toBe(true);
  ...
  it("stacks IOB carbs on top of compound carbs correctly", () => {
    ...
    // Total: 25g
    const g1 = assessReadiness(makeInput({ currentBG: 7.0, trendSlope: -0.035, iob: 0.5 }));
    expect(g1.suggestions.some((s) => s.includes("25g"))).toBe(true);

    // Compound: 20g base
    // IOB 2.5u: 2.5 * 12 = 30g
    // Total: 50g
    const g2 = assessReadiness(makeInput({ currentBG: 7.0, trendSlope: -0.035, iob: 2.5 }));
...
describe("assessReadiness — worst-case consolidated output", () => {
  it("produces clear, non-redundant suggestions in extreme scenario", () => {
    // Worst case: low BG (6.5), falling (-0.04), high IOB (0.8u), fatigued (TSB -5)
    const model = makeModel({
      activitiesAnalyzed: 5,
      targetFuelRates: [
