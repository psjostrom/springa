...
describe("buildReportCard", () => {
  ...
  it("populates entryTrend and recovery when RunBGContext provided", () => {
    ...
    const ctx: RunBGContext = {
      ...
      post: { recoveryDrop30m: -0.25, nadirPostRun: 6.0, timeToStable: 10, postRunHypo: false, endBG: 7.5, readingCount: 8, peak30m: 7.5, spike30m: 0, peak60mAboveEnd: 0 },
      totalBGImpact: -4,
    };
...
describe("scoreRecovery", () => {
  it("clean recovery (drop -0.25, nadir 6.0, no hypo) → good, Clean", () => {
    const ctx = makeCtxWithPost({ recoveryDrop30m: -0.25, nadirPostRun: 6.0, timeToStable: 10, postRunHypo: false, endBG: 7.5, readingCount: 8, peak30m: 7.5, spike30m: 0, peak60mAboveEnd: 0 });
  ...
  it("dipping (drop -0.75, nadir 4.3) → ok, Dipping", () => {
    const ctx = makeCtxWithPost({ recoveryDrop30m: -0.75, nadirPostRun: 4.3, timeToStable: 20, postRunHypo: false, endBG: 7.0, readingCount: 8, peak30m: 7.0, spike30m: 0, peak60mAboveEnd: 0 });
    const score = scoreRecovery(ctx)!;
    expect(score.rating).toBe("ok");
  ...
  it("crashed (drop -1.25, nadir 3.5, hypo) → bad, Crashed", () => {
    const ctx = makeCtxWithPost({ recoveryDrop30m: -1.25, nadirPostRun: 3.5, timeToStable: null, postRunHypo: true, endBG: 6.0, readingCount: 8, peak30m: 6.0, spike30m: 0, peak60mAboveEnd: 0 });
    const score = scoreRecovery(ctx)!;
    expect(score.rating).toBe("bad");
  ...
  it("post-hypo alone triggers bad even with mild drop", () => {
    const ctx = makeCtxWithPost({ recoveryDrop30m: -0.25, nadirPostRun: 3.8, timeToStable: null, postRunHypo: true, endBG: 7.0, readingCount: 8, peak30m: 7.0, spike30m: 0, peak60mAboveEnd: 0 });
    const score = scoreRecovery(ctx)!;
    expect(score.rating).toBe("bad");
    expect(score.label).toBe("Crashed");
  });
  ...
  it("boundary: drop exactly -0.5 → good (not ok)", () => {
    const ctx = makeCtxWithPost({ recoveryDrop30m: -0.5, nadirPostRun: 6.0, timeToStable: 10, postRunHypo: false, endBG: 7.5, readingCount: 8, peak30m: 7.5, spike30m: 0, peak60mAboveEnd: 0 });
    const score = scoreRecovery(ctx)!;
    expect(score.rating).toBe("good");
  });

  it("boundary: nadir exactly 3.9 → bad", () => {
    const ctx = makeCtxWithPost({ recoveryDrop30m: -0.5, nadirPostRun: 3.9, timeToStable: 10, postRunHypo: false, endBG: 7.5, readingCount: 8, peak30m: 7.5, spike30m: 0, peak60mAboveEnd: 0 });
    const score = scoreRecovery(ctx)!;
    expect(score.rating).toBe("bad");
  });

  it("boundary: nadir 4.5 → ok (not good)", () => {
    const ctx = makeCtxWithPost({ recoveryDrop30m: -0.75, nadirPostRun: 4.5, timeToStable: 15, postRunHypo: false, endBG: 7.0, readingCount: 8, peak30m: 7.0, spike30m: 0, peak60mAboveEnd: 0 });
    const score = scoreRecovery(ctx)!;
    expect(score.rating).toBe("ok");
  });
});
